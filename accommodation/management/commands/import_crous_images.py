...
class Command(UploadS3BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open(csv_file_path, newline="", encoding="utf-8") as csvfile:
            ...
            for row in reader:
                name = row["Résidence"].strip().title()
                print("Managing", name)
                acc_instance = (
                ...
                s3_residence_name = "_".join([k.title() for k in row["répertoire résidence"].split("_")])
                ...
                if not images:
                    ...
                    images = self.list_images_from_s3(s3_prefix)
                    if not images:
                        self.stderr.write(self.style.WARNING(f"No images found for {name}, skipping"))
                        continue
