...
class QuestionAnswerAPITests(APITestCase):
    ...
    def test_list_question_answers_by_territory(self):
        ...
        self.assertEqual(response.data[0]["title_fr"], self.qa1.title_fr)

    def test_missing_content_type(self):
        response = self.client.get(reverse("questionanswers-by-territory"), {"object_id": self.city.id})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data["detail"], "Both 'content_type' and 'object_id' parameters are required.")
    ...
    def test_invalid_content_type(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertEqual(response.data["detail"], "Invalid content_type: invalid_type")
